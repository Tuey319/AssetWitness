...
const FILTERS = ['All', 'Disputed', 'Resolved'] as const;
type Filter = typeof FILTERS[number];

const VCFG = {
  Disputed: { label: 'Disputed', dot: '#F87171', text: '#F87171', bg: 'rgba(248,113,113,0.10)', border: 'rgba(248,113,113,0.25)' },
  Resolved: { label: 'Resolved', dot: '#34D399', text: '#34D399', bg: 'rgba(52,211,153,0.10)',  border: 'rgba(52,211,153,0.25)'  },
};

// Light-mode overrides for status colors (always vibrant, theme-independent)
const VCFG_LIGHT = {
  Disputed: { label: 'Disputed', dot: '#EF4444', text: '#B91C1C', bg: '#FEF2F2', border: 'rgba(239,68,68,0.25)' },
  Resolved: { label: 'Resolved', dot: '#059669', text: '#065F46', bg: '#ECFDF5', border: 'rgba(5,150,105,0.25)' },
};
...
export default function HistoryScreen() {
  ...
  const filtered = CASES.filter(c => {
    const status = c.needsDisputeResolution ? 'Disputed' : 'Resolved';
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
      ...
      <View style={{ paddingHorizontal: 20, paddingTop: 24, paddingBottom: 16, backgroundColor: C.bg }}>
        ...
        <View style={{ backgroundColor: C.surface, borderRadius: 16, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, gap: 10, borderWidth: 1, borderColor: C.border, height: 48, marginBottom: 12 }}>
          ...
          <TextInput
            ...
            placeholder="Search handovers or items…"
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 80 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
          {[
            { l: 'Total handovers', v: String(CASES.length) },
            { l: 'DAD responsibility', v: `฿${totalDadResponsibility.toLocaleString()}` },
            { l: 'Dispute rate', v: `${disputeRate}%` },
          ].map((s, i) => (
        ...
        {filtered.length === 0 ? (
          <View style={{ alignItems: 'center', paddingVertical: 56 }}>
            ...
            <Text style={{ fontSize: 17, fontWeight: '700', color: C.ink, marginBottom: 6 }}>No handovers found</Text>
            <Text style={{ fontSize: 14, color: C.ink3, textAlign: 'center', lineHeight: 20 }}>
              {filter !== 'All' ? `No ${filter.toLowerCase()} handovers yet` : 'Try a different search term'}
            </Text>
            <Pressable
              onPress={() => router.push('/new-case')}
              style={{ marginTop: 20, backgroundColor: C.amber, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 999 }}
            >
              <Text style={{ fontSize: 14, fontWeight: '700', color: '#0C0A07' }}>Start your first handover →</Text>
            </Pressable>
          </View>
        ) : (
          filtered.map(c => {
            const v = vcfg[c.needsDisputeResolution ? 'Disputed' : 'Resolved'];
            return (
              <Pressable key={c.id} onPress={() => router.push('/results')} style={{ backgroundColor: C.surface, borderRadius: 20, padding: 18, marginBottom: 12, borderWidth: 1, borderColor: C.border }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 12, borderTopWidth: 1, borderTopColor: C.border2 }}>
                  <Text style={{ fontSize: 13, color: C.ink2 }}>
                    Estimated: <Text style={{ fontWeight: '800', color: C.ink }}>฿{c.totalEstimatedCost.toLocaleString()}</Text>
                  </Text>
                  {c.totalDadResponsibility > 0 && (
                    <View style={{ backgroundColor: isDark ? 'rgba(52,211,153,0.10)' : '#ECFDF5', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 }}>
                      <Text style={{ fontSize: 13, fontWeight: '800', color: C.ok }}>DAD ฿{c.totalDadResponsibility.toLocaleString()}</Text>
                    </View>
                  )}
                </View>
              </Pressable>
