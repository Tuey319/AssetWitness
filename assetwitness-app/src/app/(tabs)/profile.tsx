import { ChevronRight, Globe, Info, Moon, Save, Sun, User, Zap } from 'lucide-react-native';
...
const AGENTS = [
  {
    ...
    calls:    142,
    desc:     'Compares prior vs current condition photos using multi-image reasoning. Detects change type: PRE_EXISTING, NORMAL_WEAR, UNCHANGED, NEW_DAMAGE.',
    tags:     ['Vision', 'Groq', 'Llama-4'],
  },
  {
    id:       '02',
    name:     'Agreement Parser',
    ...
    status:   'online' as const,
    calls:    138,
    desc:     'Extracts responsibility_map, agreement_summary, and non_compliant_clauses from occupancy/fit-out agreements against the State Property Act and MOF Regulation.',
    tags:     ['Thai NLP', 'Typhoon', 'Legal'],
  },
  {
    id:       '03',
    name:     'Asset Policy Reasoning',
    model:    'typhoon-v2.5-30b-a3b-instruct + ChromaDB',
    provider: 'OpenTyphoon + Local',
    type:     'RAG · asset policy corpus',
    context:  'Top-3 retrieval',
    latency:  '6.2s avg',
    color:    '#34D399',
    status:   'online' as const,
    calls:    274,
    desc:     'Retrieves relevant state-property policy via ChromaDB semantic search, then reasons per-item responsibility: NORMAL_WEAR / OCCUPANT_RESPONSIBILITY / DAD_RESPONSIBILITY / DISPUTED.',
  ...
  },
  {
    ...
    status:   'online' as const,
    calls:    127,
    desc:     'Generates a condition certification report, fit-out completion checklist, and liability summary as Thai-language PDFs using ReportLab Platypus with Leelawadee font.',
...
const POLICY_DB = [
  {
    title:   'State Property Act B.E. 2562',
    detail:  'พ.ร.บ. ที่ราชพัสดุ · scope & delegation of state-property governance',
    chunks:  2,
    color:   '#60A5FA',
  },
  {
    title:   'MOF Regulation B.E. 2552',
    detail:  'ระเบียบกระทรวงการคลัง · leasing/benefit-generation, 30-day return notice',
    chunks:  3,
    color:   '#34D399',
  },
  {
    title:   'DAD Policy — pending confirmation',
    detail:  'Placeholder chunks for DAD-specific wear/fit-out thresholds not yet public',
    chunks:  2,
    color:   '#F59E0B',
  },
];
...
export default function ProfileScreen() {
  ...
  const totalDadResponsibility = cases.reduce((s, c) => s + c.totalDadResponsibility, 0);
  const disputeRate = cases.length > 0 ? Math.round(cases.filter(c => c.needsDisputeResolution).length / cases.length * 100) : 0;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 80 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ backgroundColor: C.surface, marginHorizontal: 20, marginTop: 24, borderRadius: 24, padding: 24, borderWidth: 1, borderColor: C.border, marginBottom: 24, overflow: 'hidden', position: 'relative' }}>
          ...
          {editing ? (
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {[
                { v: String(cases.length), l: 'Handovers' },
                { v: totalDadResponsibility >= 1000 ? `฿${Math.round(totalDadResponsibility / 1000)}k` : `฿${totalDadResponsibility}`, l: 'DAD resp.' },
                { v: cases.length > 0 ? `${disputeRate}%` : '—', l: 'Disputed' },
              ].map((s, i) => (
        ...
        {/* ── Asset policy database ────────────────────────── */}
        <Text style={{ fontSize: 10, color: C.ink3, fontWeight: '700', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 10, paddingHorizontal: 20 }}>Asset policy database</Text>
        <View style={{ marginHorizontal: 20, marginBottom: 24, gap: 8 }}>
          {POLICY_DB.map(db => (
            <View key={db.title} style={{ backgroundColor: C.surface, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: C.border }}>
              ...
              <Text style={{ fontSize: 12, color: C.ink3 }}>{db.detail}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={{ backgroundColor: C.surface, borderRadius: 20, marginHorizontal: 20, borderWidth: 1, borderColor: C.border, overflow: 'hidden', marginBottom: 28 }}>
          <SettingsRow icon={Zap} iconColor={C.warn} label="AssetWitness v1.0" sub="Proposed for Sustainnovation 2026" />
          <SettingsRow icon={Info} iconColor={C.blue} label="Built with" sub="Expo · Groq · Typhoon v2 · ChromaDB · ReportLab" />
        </View>

        <View style={{ alignItems: 'center', gap: 4 }}>
          <Text style={{ fontSize: 11, color: C.ink3 }}>Sustainnovation 2026 · Team: KP · Beam · Tuey</Text>
          <View style={{ marginTop: 6, backgroundColor: C.amberSoft, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 999, borderWidth: 1, borderColor: C.border }}>
            <Text style={{ fontSize: 10, color: C.amberDark, fontWeight: '700' }}>Derived from RoomWitness · BDI Bangkok Hackathon 2026</Text>
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
