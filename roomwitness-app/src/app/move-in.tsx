...
export default function MoveInScreen() {
  const C = getColors(useStore(s => s.theme));
  const addMoveInRecord = useStore(s => s.addMoveInRecord);
  ...
  function save() {
    if (photos.length === 0) {
      Alert.alert('Add at least one photo', 'Photograph each room/item before saving.');
      return;
    }
    setSaving(true);
    addMoveInRecord({
    ...
    Alert.alert(
      'Saved — free of charge',
      "We'll keep these safe on your device until you move out, even if that's a year from now. You won't need to re-shoot this.",
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
      <View style={{ backgroundColor: C.surface, paddingHorizontal: 20, paddingTop: 14, paddingBottom: 20, borderBottomWidth: 1, borderBottomColor: C.border }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <View style={{ backgroundColor: C.okBg, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999, borderWidth: 1, borderColor: C.border }}>
            <Text style={{ fontSize: 11, color: C.ok, fontWeight: '700' }}>Free · no AI used yet</Text>
          </View>
        </View>
        <Text style={{ fontSize: 28, fontWeight: '900', color: C.ink, letterSpacing: -1, lineHeight: 32 }}>Document your{'\n'}move-in</Text>
        <Text style={{ fontSize: 13, color: C.ink2, marginTop: 6 }}>Do this the day you get your keys — it's the single strongest piece of evidence for a deposit dispute later.</Text>
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingBottom: 24 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={{ backgroundColor: C.surface, borderRadius: 20, padding: 20, borderWidth: 1, borderColor: C.border, marginBottom: 16 }}>
            <DarkInput label="Property / unit nickname" value={label} onChange={setLabel} placeholder="e.g. Ideo Mobi Room 12B" />

            <Text style={{ fontSize: 10, fontWeight: '700', color: C.ink3, letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: 10 }}>Move-in photos</Text>
          ...
          <View style={{ backgroundColor: C.okBg, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: C.okBorder, flexDirection: 'row', gap: 10 }}>
            ...
            <Text style={{ flex: 1, fontSize: 13, color: C.ink2, lineHeight: 20 }}>
              Stored only on this device. Nothing is sent to the AI or charged until you actually file a
              deposit claim at move-out.
            </Text>
          </View>
        </ScrollView>
      ...
      <View style={{ paddingHorizontal: 20, paddingBottom: 20, paddingTop: 12, backgroundColor: C.surface, borderTopWidth: 1, borderTopColor: C.border }}>
        <Pressable onPress={save} disabled={saving}
          ...
          <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '900', letterSpacing: -0.3 }}>Save move-in record — free</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
