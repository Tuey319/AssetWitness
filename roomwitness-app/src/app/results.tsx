...
function VerdictBadge({ v }: { v: 'LAWFUL' | 'DISPUTED' | 'UNLAWFUL' }) {
  ...
  const cfg = {
    LAWFUL:   { bg: C.okBg,     text: C.ok,     dot: C.ok,     label: 'Lawful'   },
    DISPUTED: { bg: C.warnBg,   text: C.warn,   dot: C.warn,   label: 'Disputed' },
    UNLAWFUL: { bg: C.dangerBg, text: C.danger, dot: C.danger, label: 'Unlawful' },
  }[v];
  return (
      ...
      {open && (
        <View style={{ paddingHorizontal: 16, paddingBottom: 16, gap: 8, borderTopWidth: 1, borderTopColor: C.border2, paddingTop: 12 }}>
          {[
            ...
            <View key={k} style={{ backgroundColor: C.surface2, borderRadius: 12, padding: 12 }}>
        ...
        <View style={{ backgroundColor: C.ink, borderRadius: 24, padding: 24, marginBottom: 16 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 12 }}>
            ...
            <View style={{ flex: 1, backgroundColor: 'rgba(52,211,153,0.15)', borderRadius: 16, padding: 16 }}>
              <Text style={{ fontSize: 11, color: 'rgba(52,211,153,0.8)', fontWeight: '600', marginBottom: 6 }}>UNLAWFUL ({pct}%)</Text>
              <Text style={{ fontSize: 22, fontWeight: '800', color: '#34D399' }}>฿{(total_unlawful_thb || recov).toLocaleString()}</Text>
            ...
            {unfair_clauses.length > 0 && (
              <View style={{ marginTop: 12, backgroundColor: C.dangerBg, borderRadius: 14, padding: 14 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                  ...
                  <Text style={{ fontSize: 13, fontWeight: '700', color: C.danger }}>Void clauses found ({unfair_clauses.length})</Text>
                </View>
                {unfair_clauses.map((c: any, i: number) => (
                  <View key={i} style={{ marginBottom: 8 }}>
                    ...
                    <Text style={{ fontSize: 12, color: C.danger, marginTop: 2 }}>{c.reason_void}</Text>
                  </View>
                ))}
              </View>
            )}
